...
contract ChallengeContract {
    ...
    function createChallenge(
        ...
        require(_expiryDate > block.timestamp, "Expiry Date is already finished");
        ...
        emit ChallengeCreated(_challengeId, _betAmount, _expiryDate, _isPublicChallenge);
    }
    ...
    function addUserToChallenge(uint32 _challengeId) external payable {
        ...
        challenges[_challengeId].totalAcceptedUsers = 2;
        challenges[_challengeId].acceptedUser = msg.sender;
        listOfUsersInChallenge[_challengeId].push(msg.sender);
        userChallenges[msg.sender].push(_challengeId);
    ...
    function calculateReward(uint32 _challengeId) public {
        ...
        challenges[_challengeId].isActive = false;
        challenges[_challengeId].totalWinners = uint8(winnerList[_challengeId].length);
    }

    function addWinners(uint32 _challengeId, address _winner) external onlyOwner {
        ...
        require(
            winnerList[_challengeId].length <= challenges[_challengeId].totalAcceptedUsers
        );
        require(
    ...
    function listOfAcceptedUsers(
        ...
        for (uint8 i = 0; i < listOfUsersInChallenge[_challengeId].length; i++) {
    ...
    function claimReward(uint32 _challengeId) external payable noReentrancy {
        ...
        (bool success, ) = msg.sender.call{value: winnerAmount[msg.sender][_challengeId]}(
            ""
        );
    ...
    function fetchUserChallenges(
        ...
        uint[] memory senderArray = new uint[](userChallenges[userAddress].length);
    ...
    function getAllChallenges() public view returns (Challenge[] memory) {
        Challenge[] memory senderArray = new Challenge[](listOfChallenges.length);
